...
Chronological assessment requires addressing two distinct disruption patterns: global structural violations where distant segments swap positions, and local flow disruptions where neighboring segments exchange order.

Chronological assessment requires two complementary perspectives to capture different types of temporal disruptions. A global perspective detects large-scale structural violations where narrative segments appear far from their expected positions. A local perspective captures fine-grained flow disruptions where neighboring segments violate expected sequential relationships.

Neither approach alone provides comprehensive chronological evaluation. Global assessment may miss subtle neighbor-level reorderings, while local assessment may overlook dramatic structural violations. This necessitates dual assessment mechanisms that together capture both macro-level chronological structure and micro-level narrative flow.

NAS addresses chronological evaluation through integrated assessment components that capture different aspects of temporal alignment and narrative coherence.

\subsubsection{Defining Local Narrative Tolerance (LNT)}
Video narratives permit multiple valid orderings and descriptive variations without compromising comprehension quality. Complex scenes with concurrent events and overlapping actions create legitimate alternatives in temporal presentation and detail granularity.
...
To accommodate this inherent narrative flexibility, VCS employs Local Narrative Tolerance (LNT) as a configurable tolerance parameter. LNT defines acceptable deviation from strict chronological ordering while maintaining narrative coherence. LNT distinguishes legitimate narrative variations from chronological errors, providing configurable evaluation strictness.
...
The critical threshold occurs at 50% similarity matrix coverage. When only 2 chunks exist on either axis, mapping windows covering 50% or more of the comparison area indicate that chronological constraints have become too permissive to provide meaningful evaluation. Beyond this threshold, the windows are so expansive that even severely disordered narratives would appear chronologically acceptable.
