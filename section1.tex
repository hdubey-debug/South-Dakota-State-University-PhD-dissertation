...
\paragraph{Mapping Window Construction}
Let:
\begin{align}
N_{ref} &= |C_{ref}|, \quad N_{gen} = |C_{gen}| \\
s &= \frac{\max(N_{ref}, N_{gen})}{\min(N_{ref}, N_{gen})} \\
h_{mw} &= \lceil s \rceil
\end{align}

Assume $N_{ref} \geq N_{gen}$ (swap roles otherwise). For every generated index $j \in \{0, \ldots, N_{gen} - 1\}$:
\begin{equation}
MW_{prec}(j) = \left[\lfloor j \cdot s \rfloor, \min(\lfloor j \cdot s \rfloor + h_{mw}, N_{ref})\right)
\end{equation}
This defines a precision mapping window in the reference timeline that a well-aligned match for $j$ should inhabit.

A dual recall window for each reference index $i$ is the smallest closed range of $j$-indices whose precision windows cover $i$:
\begin{equation}
MW_{rec}(i) = \left[\min\{j \mid i \in MW_{prec}(j)\}, \max\{j \mid i \in MW_{prec}(j)\} + 1\right)
\end{equation}

VCS assigns Precision Windows ($MW_{prec}$) and Recall Windows ($MW_{rec}$) based on sequence direction: when $N_{ref} \geq N_{gen}$, precision uses direct windows and recall uses reverse windows; otherwise, assignments are reversed.

\subsubsection{Best Matching Algorithm}
Having established Mapping Windows within the similarity matrix $\mathbf{S}$, VCS pairs each generated chunk with its best reference counterpart, and vice versa. Naively selecting the highest-similarity reference for each generated chunk leads to semantic collision and semantic ambiguity. VCS resolves these issues through a two-step Best-Matching Algorithm.

Given the cosine-similarity matrix $\mathbf{S} \in [0,1]^{N_{ref} \times N_{gen}}$, a context-aware best match for each generated index $j$ is chosen as follows:

\paragraph{Context Filtering}
Let:
\begin{align}
m_j &= \max_i S_{i,j} \\
\alpha &\in (0,1) \quad \text{(default 0.6)} \\
\kappa &> 0 \quad \text{(default 5.0)}
\end{align}

Define:
\begin{align}
\text{context\_range} &= 1 - \alpha \\
w_j &= \frac{\text{context\_range} - (1 - m_j)}{\kappa \cdot m_j} \quad \text{(clamped at 0)} \\
\theta_j &= m_j - w_j
\end{align}

The candidate set is:
\begin{equation}
C_j = \{i \mid S_{i,j} \geq \theta_j\}
\end{equation}

\paragraph{Window-Constrained Selection}
If $|C_j| = 1$ or no mapping window is supplied, pick the unique/maximal-similarity index. Otherwise, let $MW_{prec}(j) = [a_j, b_j)$. For every $i \in C_j$ define its distance to the window:
\begin{equation}
d_j(i) = \begin{cases}
0, & i \in MW_{prec}(j) \\
\min(|i - a_j|, |i - (b_j - 1)|), & \text{otherwise}
\end{cases}
\end{equation}

Select:
\begin{equation}
i_j^* = \arg\max_{i \in C_j} (-d_j(i), S_{i,j})
\end{equation}
i.e., the candidate with minimum distance, breaking ties by larger similarity.

The same routine is run row-wise on $\mathbf{S}$ with the recall windows $MW_{rec}(i)$ to produce the bidirectional match sets:
\begin{align}
M_P &= \{(j, i_j^*)\} \quad \text{(precision best matches)} \\
M_R &= \{(j_i^*, i)\} \quad \text{(recall best matches)}
\end{align}

\subsubsection{Local Alignment Score (LAS)}
Having obtained the correspondence sets $M_P$ and $M_R$ from the Best-Matching Algorithm, VCS computes the LAS by averaging cosine similarities of matched chunk pairs. For each orientation we average the similarities of the accepted pairs:
\begin{align}
LAS_{prec} &= \frac{1}{|M_P|} \sum_{(j,i) \in M_P} S_{i,j} \\
LAS_{rec} &= \frac{1}{|M_R|} \sum_{(j,i) \in M_R} S_{i,j}
\end{align}

Finally:
\begin{equation}
LAS = \frac{2 \cdot LAS_{prec} \cdot LAS_{rec}}{LAS_{prec} + LAS_{rec}}
\end{equation}
(The harmonic mean; empty vectors default to 0)
...
\subsubsection{Scaled Semantic Alignment Score (SAS)}
Let $GAS \in [0,1]$ and $LAS \in [0,1]$. Define:
\begin{equation}
v = GAS - (1 - LAS) = GAS + LAS - 1
\end{equation}

If either (i) $LAS = 0$ or (ii) $v \leq 0$ (equivalently $GAS + LAS \leq 1$), the score is clipped to 0. Otherwise:
\begin{equation}
SAS = \frac{GAS + LAS - 1}{LAS}
\end{equation}

\subsubsection{Scaled Video Comprehension Score (VCS)}
Let:
\begin{align}
s &= SAS \\
n &= NAS_{final} \\  
...
If $u \leq 0$ (i.e., $s + n \leq 1$) or $b = 0$, the score is 0. Otherwise:
\begin{equation}
\boxed{VCS = \frac{s + n - 1}{\max(s, n)}}
\end{equation}

This formulation ensures that high VCS scores require both strong semantic alignment (through SAS) and strong narrative coherence (through NAS), with the final score normalized by the stronger component to prevent inflation when one component dominates.
