...
The successive vertical differences $\Delta y_i^{\text{floor}}$ along the floor path are cached in a map $\text{DY}_{\text{floor}}[x_i] = \Delta y_i^{\text{floor}}$; they later cap oversized jumps when Local Narrative Tolerance (LNT) is enabled.
...
\end{equation}
From this, the LNT window $\omega_0$ and its expanded version $\omega_{LNT}$ are set exactly as in the code:
\begin{equation}
\omega_0 = \begin{cases}
h_{mw}, & L_t \leq L_s, \\
...
\end{cases}
\end{equation}
where $\rho = L_t/L_s - \lfloor L_t/L_s \rfloor$ and $\tau_{LNT} \in \mathbb{N}_{\geq 0}$ is user-set.
...
\end{equation}
\item \textbf{LNT-capped segment} if $\omega_0 < |\Delta y_i|^* \leq \omega_{LNT}$ (requires $\tau_{LNT} > 0$):
\begin{equation}
...
Let $N_{ref}$ and $N_{gen}$ be the reference- and generated-lengths, $\text{MW}_{P}$ and $\text{MW}_{R}$ the Mapping-Window lists produced earlier, each window $w_j = [s_j, e_j)$ having height $h_j = e_j - s_j$.
...
\text{MW}_{R}, & N_{ref} < N_{gen}, \\
\text{MW}_{P}, & N_{ref} \geq N_{gen}.
\end{cases}
\end{equation}
